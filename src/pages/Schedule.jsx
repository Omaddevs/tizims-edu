...
const LEGEND = [
  { id: 'attended', label: 'Qatnashilgan', icon: CheckCircle2, dot: 'text-emerald-500', block: 'border-emerald-200 bg-emerald-50 text-emerald-800' },
  { id: 'unexcused', label: 'Sababsiz', icon: XCircle, dot: 'text-rose-500', block: 'border-rose-200 bg-rose-50 text-rose-800' },
  { id: 'excused', label: 'Sababli', icon: AlertTriangle, dot: 'text-amber-500', block: 'border-amber-200 bg-amber-50 text-amber-800' },
]
const LEGEND_BY_ID = Object.fromEntries(LEGEND.map((l) => [l.id, l]))
const DEFAULT_BLOCK = 'border-brand-200 bg-brand-50 text-brand-800'
...
function DesktopScheduleView({ me, schedule, users, groups, attendance }) {
  ...
  const lastHour = lessonsByDay.flat().reduce((max, s) => Math.max(max, timeToMinutes(s.end) / 60), 0)
  const nowHour = today.getHours() + today.getMinutes() / 60
  const dayEnd = Math.max(13, Math.ceil(nowHour + 0.4), totalLessons ? Math.ceil(lastHour + 0.15) : 0)
  ...
  return (
    <div className="pb-8">
      ...
      <div className="mt-1.5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            ...
            className={cn(
              'rounded-[10px] px-3.5 py-2 text-[13px] font-semibold',
          ...
          <span className="ml-2 mr-1 text-[13px] font-semibold text-[#2b3340]">{rangeLabel}</span>
          <button
            type="button"
            onClick={goPrev}
            className="grid h-8 w-8 place-items-center rounded-full bg-white text-[#9aa3b2] shadow-[0_2px_8px_rgba(15,23,42,0.06)]"
            aria-label="Oldingi"
          >
            <ChevronLeft size={15} />
          </button>
          <button
            type="button"
            onClick={goToday}
            className="rounded-[10px] bg-[#2f80ed] px-3.5 py-2 text-[13px] font-semibold text-white"
          >
            Bugun
          </button>
          <button
          ...
          <button
            ...
            className="ml-0.5 rounded-[10px] bg-gradient-to-b from-[#5ad0f6] to-[#4f6ef7] px-2 py-1.5 text-[10px] font-extrabold tracking-wide text-white"
          >
            Support
          </button>
        </div>
      ...
      <div className="mt-2.5 overflow-x-auto rounded-[16px] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        <div className="min-w-[720px]">
        <div className="flex">
          <div className="grid w-[64px] shrink-0 place-items-center border-b border-r border-[#eef1f6] py-[13px]">
          ...
          {days.map((d) => {
            ...
            return (
              <div
                key={iso}
                className={cn(
                  'flex flex-1 items-center justify-center gap-1.5 border-b border-r border-[#eef1f6] py-[11px] last:border-r-0',
                  isToday && 'bg-[#eaf1ff]',
                )}
              >
                <span
        ...
        <div className="relative flex overflow-hidden">
          <div className="relative w-[64px] shrink-0 border-r border-[#eef1f6]" style={{ height: dayHeight }}>
            {hourLabels.map((h) => (
              <span
                key={h}
          ...
          {days.map((d, di) => {
            ...
            return (
              <div
                key={iso}
                className={cn('relative flex-1 border-r border-[#eef1f6] last:border-r-0', isToday && 'bg-[#f3f7ff]')}
                style={{
                  height: dayHeight,
